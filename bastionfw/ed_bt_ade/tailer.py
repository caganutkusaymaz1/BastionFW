...
# Upper bound for the partial-line buffer. A single "line" larger than this
# (a hostile or corrupted writer) must not pin unbounded memory; the head is
# dropped and only the most recent MAX_BUFFER_BYTES are retained.
...
class AsyncLogTailer:
    ...
    async def _read_handle(self) -> None:
        ...
        if len(self._buffer) > MAX_BUFFER_BYTES:
            # Truncate from the head. The dropped bytes may cut a record, but
            # retaining the tail keeps the most recent data and bounds memory
            # even when a source writes a gigabyte without a newline.
            dropped = len(self._buffer) - MAX_BUFFER_BYTES
            self._buffer = self._buffer[-MAX_BUFFER_BYTES:]
            LOGGER.warning("tailer buffer exceeded %d bytes; dropped %d bytes",
                           MAX_BUFFER_BYTES, dropped,
                           extra={"source": self.source.name, "event": "buffer_truncated"})
