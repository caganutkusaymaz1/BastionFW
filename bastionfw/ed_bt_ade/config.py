...
class LogSource:
    ...
    @property
    def is_coraza_audit(self) -> bool:
        """True when the source is a Coraza WAF JSON audit log."""
        return self.source_type == "coraza_audit"
...
class DetectionConfig:
    ...
    coraza_block_score: int = 5


@dataclass(frozen=True, slots=True)
class WafConfig:
    """Coraza WAF integration: monitoring, fail-open, and dynamic deny-list.

    ``enabled`` gates both the bidirectional deny-list driver and the deadman
    switch. Keep it disabled (the default) until the WAF stack and the
    firewall backend have been validated in staging.
    """

    enabled: bool = False
    mode: str = "detect"
    data_dir: Path = Path("state/waf")
    health_url: str = ""
    ...
    def deny_list_path(self) -> Path:
        return self.data_dir / "denylist.json"

    @property
    def deny_include_path(self) -> Path:
        return self.data_dir / "deny.caddy"

    @property
    def engine_mode_path(self) -> Path:
        return self.data_dir / "engine-mode.conf"

    @property
    def fail_open_flag_path(self) -> Path:
        return self.data_dir / "fail-open.flag"
...
def load_config(path: str | Path, environ: dict[str, str] | None = None) -> AppConfig:
    ...
    for index, item in enumerate(raw_sources):
        ...
        source_type = str(item.get("source_type", "plain")).lower()
        if source_type not in {"plain", "coraza_audit"}:
            raise ConfigError("log_sources[].source_type must be 'plain' or 'coraza_audit'")
        sources.append(
    ...
    detection = DetectionConfig(
        ...
        coraza_block_score=_positive(detection_raw.get("coraza_block_score", 5),
                                     "detection.coraza_block_score", integer=True),
    )
    ...
    waf_enabled = _as_bool(waf_raw.get("enabled", False), "waf.enabled")
    waf_mode = str(waf_raw.get("mode", "detect")).lower()
    if waf_mode not in {"detect", "block"}:
        raise ConfigError("waf.mode must be 'detect' or 'block'")
    waf_data_dir = Path(str(waf_raw.get("data_dir", "state/waf")))
    raw_caddyfile = waf_raw.get("caddyfile_path")
    caddyfile = (Path(raw_caddyfile)
                 if isinstance(raw_caddyfile, str) and raw_caddyfile else None)
    waf = WafConfig(
        enabled=waf_enabled,
        mode=waf_mode,
        data_dir=waf_data_dir,
        health_url=_validated_url(waf_raw.get("health_url", ""),
                                  "waf.health_url", allow_empty=True),
        health_interval_seconds=_positive(
            waf_raw.get("health_interval_seconds", 10.0),
            "waf.health_interval_seconds"),
        failure_threshold=_positive(waf_raw.get("failure_threshold", 3),
                                    "waf.failure_threshold", integer=True),
        admin_url=_validated_url(waf_raw.get("admin_url", ""),
                                 "waf.admin_url", allow_empty=True),
        caddyfile_path=caddyfile,
        rollback_command=str(waf_raw.get("rollback_command", "")).strip(),
    )
    ...
    if "ED_BT_ADE_WAF_ENABLED" in env:
        waf_enabled = _as_bool(env["ED_BT_ADE_WAF_ENABLED"], "ED_BT_ADE_WAF_ENABLED")
    if "ED_BT_ADE_WAF_MODE" in env:
        candidate = str(env["ED_BT_ADE_WAF_MODE"]).lower()
        if candidate not in {"detect", "block"}:
            raise ConfigError("ED_BT_ADE_WAF_MODE must be 'detect' or 'block'")
        waf_mode = candidate
    ...
    return AppConfig(
        ...
        waf=WafConfig(
            ...
            mode=waf_mode,
            data_dir=waf_data_dir,
            health_url=waf.health_url,
            health_interval_seconds=waf.health_interval_seconds,
            failure_threshold=waf.failure_threshold,
            admin_url=waf.admin_url,
            caddyfile_path=caddyfile,
            rollback_command=waf.rollback_command,
        ),
